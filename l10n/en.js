OC.L10N.register(
    "worktime",
    {
    "Abläufe" : "Workflows",
    "Aktuell gültiger Wert aus dem Arbeitszeitprofil. Zum Ändern unten das Profil bearbeiten oder ein neues anlegen." : "Currently valid value from the work schedule profile. To change it, edit the profile below or create a new one.",
    "Eingereicht – wartet auf Genehmigung" : "Submitted – awaiting approval",
    "Einstellungs-Navigation" : "Settings navigation",
    "Fehler beim Laden der Monatseinträge" : "Failed to load month entries",
    "Filter" : "Filter",
    "Firma" : "Company",
    "ID" : "ID",
    "Kalender" : "Calendar",
    "Liste" : "List",
    "Monat genehmigt – Einträge gesperrt. Korrektur nur durch HR." : "Month approved – entries locked. Corrections only by HR.",
    "Möchten Sie die Zeiteinträge für {month} einreichen? Die eingereichten Einträge werden zur Genehmigung übermittelt." : "Do you want to submit the time entries for {month}? The submitted entries will be sent for approval.",
    "Stammdaten und Standardwerte, die für neue Mitarbeiter vorausgewählt werden." : "Master data and default values pre-selected for new employees.",
    "Standard-Urlaubstage" : "Default vacation days",
    "Standard-Wochenstunden" : "Default weekly hours",
    "Wird geladen…" : "Loading…",
    "Wochenstunden und Urlaubstage ergeben sich aus dem Arbeitszeitprofil unten und werden dort gepflegt." : "Weekly hours and vacation days are derived from the work schedule profile below and are maintained there.",
    "Audit-Eintrag" : "Audit entry",
    "Feld" : "Field",
    "Vorher" : "Before",
    "Nachher" : "After",
    "Keine Detaildaten vorhanden." : "No detail data available.",
    "Schließen" : "Close",
    "Details anzeigen" : "Show details",
    "Aktuell sind keine Team-Abwesenheiten sichtbar." : "No team absences are currently visible.",
    "Aktuell wartet nichts auf Ihre Genehmigung." : "Nothing is currently awaiting your approval.",
    "Begründung" : "Reason",
    "Bestätigte Abwesenheit." : "Confirmed absence.",
    "Details" : "Details",
    "Färbung" : "Colouring",
    "Keine offenen Genehmigungen" : "No pending approvals",
    "Monat zurückweisen" : "Reject month",
    "{count} Einträge genehmigt – PDF wird im Hintergrund archiviert (kann einige Minuten dauern)." : "{count} entries approved – PDF is being archived in the background (may take a few minutes).",
    "Archivierungs-Status" : "Archiving status",
    "PDF nach dem Genehmigen sofort archivieren" : "Archive the PDF immediately after approval",
    "{count} Einträge genehmigt – PDF im Archiv erstellt." : "{count} entries approved – PDF created in the archive.",
    "{count} Einträge genehmigt – PDF im Archiv aktualisiert." : "{count} entries approved – PDF updated in the archive.",
    "Genehmigt, aber Archivierung fehlgeschlagen." : "Approved, but archiving failed.",
    "Zuletzt archiviert" : "Recently archived",
    "Die PDF-Archivierung läuft über einen Hintergrund-Job (ca. alle 5 Minuten). Hier sehen Sie ausstehende und fehlgeschlagene Archivierungen." : "PDF archiving runs via a background job (roughly every 5 minutes). Here you see pending and failed archiving runs.",
    "{pending} ausstehend · {failed} fehlgeschlagen" : "{pending} pending · {failed} failed",
    "Erneut versuchen" : "Retry",
    "Aktualisieren" : "Refresh",
    "Archivierung wird erneut versucht" : "Archiving will be retried",
    "Erneuter Versuch fehlgeschlagen" : "Retry failed",
    "PDF-Archivierung für %1$s (%2$s) ist fehlgeschlagen" : "PDF archiving for %1$s (%2$s) failed",
    "Fehler: %s" : "Error: %s",
    "Warum wird der Monat zurückgewiesen?" : "Why is the month being rejected?",
    "{count} Einträge zurückgewiesen" : "{count} entries rejected",
    "Fehler beim Zurückweisen" : "Error while rejecting",
    "Monatsdetail" : "Monthly detail",
    "Nach Art" : "By type",
    "Nach Status" : "By status",
    "Offen: {n}" : "Open: {n}",
    "Offener Antrag, noch nicht genehmigt." : "Open request, not yet approved.",
    "Resturlaub" : "Remaining leave",
    "Team gesamt" : "Team total",
    "Urlaub genommen" : "Leave taken",
    "Warum wird die Genehmigung zurückgenommen?" : "Why is the approval being withdrawn?",
    "Zurückweisen" : "Reject",
    "Überstunden gesamt" : "Total overtime",
    "✓ aktuell" : "✓ up to date",
    "PDF über Zeitraum …" : "PDF for date range …",
    "Arbeitszeitnachweis über Zeitraum" : "Timesheet for a date range",
    "Erzeugt einen Arbeitszeitnachweis als PDF für einen frei wählbaren Zeitraum (z. B. vom 20. bis zum 20.)." : "Generates a timesheet PDF for a freely selectable date range (e.g. from the 20th to the 20th).",
    "Das Enddatum darf nicht vor dem Startdatum liegen." : "The end date must not be before the start date.",
    "PDF erstellen" : "Create PDF",
    "Projekt ist erforderlich." : "Project is required.",
    "Beschreibung ist erforderlich." : "Description is required.",
    "Wochenstunden müssen größer als 0 sein. Für eine vorübergehende Auszeit (z. B. Elternzeit) eine Abwesenheit erfassen statt 0 Stunden." : "Weekly hours must be greater than 0. For a temporary leave (e.g. parental leave) record an absence instead of 0 hours.",
    "0 Arbeitstage in diesem Monat – vermutlich fehlt ein gültiges Arbeitszeitprofil mit Wochenstunden größer als 0. Bitte das Arbeitszeitprofil des Mitarbeiters prüfen." : "0 working days this month – a valid work schedule with weekly hours greater than 0 is probably missing. Please check the employee's work schedule.",
    "Auswertung" : "Evaluation",
    "Aggregiert" : "Aggregated",
    "Projekt suchen …" : "Search project …",
    "Mitarbeiter suchen …" : "Search employee …",
    "+ {count} weitere" : "+ {count} more",
    "Alle" : "All",
    "Für diese Auswahl liegen keine Buchungen vor." : "No bookings for this selection.",
    "Einzelbuchungen" : "Individual bookings",
    "CSV" : "CSV",
    "PDF" : "PDF",
    "Zurück" : "Back",
    "Weiter" : "Next",
    "Gebuchte Stunden" : "Booked hours",
    "Mitarbeitende" : "Employees",
    "Anteil" : "Share",
    "Unbekannt" : "Unknown",
    "Quartal" : "Quarter",
    "Fehler beim Laden der Auswertung" : "Error loading the evaluation",
    "+ {days} beantragt" : "+ {days} requested",
    "Abwesenheit" : "Absence",
    "Ansicht" : "View",
    "Eingereicht – Bearbeitung erst nach Genehmigung oder Ablehnung möglich." : "Submitted – editing only possible after approval or rejection.",
    "Eintrag hinzufügen" : "Add entry",
    "Erfasst" : "Recorded",
    "Erfasst: {hours}" : "Recorded: {hours}",
    "Feiertag" : "Public holiday",
    "Meine Abwesenheiten" : "My absences",
    "Monat genehmigt – gesperrt. Korrektur nur durch HR." : "Month approved – locked. Corrections by HR only.",
    "Noch nichts erfasst." : "Nothing recorded yet.",
    "Jahressaldo {year}" : "Year balance {year}",
    "Soll (anteilig bis heute)" : "Target (prorated up to today)",
    "Stand heute" : "As of today",
    "Tag" : "Day",
    "Tag ausgewählt" : "Day selected",
    "Tage übrig" : "days left",
    "Urlaub {year}" : "Vacation {year}",
    "Zeiten" : "Times",
    "anteilig: im Plan" : "prorated: on track",
    "inkl. {days} Tage Übertrag" : "incl. {days} days carryover",
    "noch {hours} h bis Monatssoll" : "{hours} h left to monthly target",
    "{hours} h unter Plan" : "{hours} h behind",
    "{min} Min" : "{min} min",
    "{min} Min Pause" : "{min} min break",
    "{n} Einträge" : "{n} entries",
    "{scope} Tage" : "{scope} days",
    "{used} von {total} Tagen genommen" : "{used} of {total} days taken",
    "In „Abwesenheit\" öffnen" : "Open in “Absence”",
    "Genehmigung zurücknehmen" : "Revoke approval",
    "{count} Einträge zur Korrektur freigegeben" : "{count} entries released for correction",
    "Fehler beim Zurücknehmen der Genehmigung" : "Error revoking approval",
    "Zeiterfassung" : "Time Tracking",
    "Abwesenheiten" : "Absences",
    "Team" : "Team",
    "Einstellungen" : "Settings",
    "Übersicht" : "Overview",
    "Meine Einstellungen" : "My Settings",
    "Genehmigungen" : "Approvals",
    "Audit-Log" : "Audit Log",
    "Alle Aktionen" : "All Actions",
    "Alle Typen" : "All Types",
    "Filtern" : "Filter",
    "Zeitpunkt" : "Timestamp",
    "Benutzer" : "User",
    "Aktion" : "Action",
    "Typ" : "Type",
    "Änderung" : "Change",
    "Erstellt" : "Created",
    "Bearbeitet" : "Updated",
    "Gelöscht" : "Deleted",
    "Eingereicht" : "Submitted",
    "Genehmigt" : "Approved",
    "Abgelehnt" : "Rejected",
    "Zeiteintrag" : "Time Entry",
    "Einstellung" : "Setting",
    "Es werden maximal 200 Einträge angezeigt. Bitte Filter verwenden um die Ergebnisse einzuschränken." : "Showing a maximum of 200 entries. Use filters to narrow down the results.",
    "Willkommen bei WorkTime" : "Welcome to WorkTime",
    "Es sind noch keine Mitarbeiter eingerichtet. Legen Sie unter Einstellungen Mitarbeiter an, um zu starten." : "No employees have been set up yet. Go to Settings to add employees and get started.",
    "Zu den Einstellungen" : "Go to Settings",
    "Kein Mitarbeiterprofil" : "No Employee Profile",
    "Sie haben noch kein Mitarbeiterprofil. Bitte wenden Sie sich an Ihren Administrator oder HR-Manager, um freigeschaltet zu werden." : "You don't have an employee profile yet. Please contact your administrator or HR manager to get activated.",
    "Neuer Eintrag" : "New Entry",
    "Eintrag bearbeiten" : "Edit Entry",
    "Datum" : "Date",
    "Beginn" : "Start",
    "Ende" : "End",
    "Arbeitsbeginn" : "Work Start",
    "Arbeitsende" : "Work End",
    "Pause" : "Break",
    "Pause (Minuten)" : "Break (minutes)",
    "Arbeitszeit: {hours}" : "Working time: {hours}",
    "Projekt" : "Project",
    "Projekt auswählen" : "Select project",
    "Neues Projekt" : "New Project",
    "Projekt bearbeiten" : "Edit Project",
    "Projektcode" : "Project Code",
    "Kunde" : "Customer",
    "Optional, z.B. für die Auswertung" : "Optional, e.g. for the evaluation",
    "Buchungsberechtigung" : "Booking permission",
    "Inaktive Projekte stehen nicht mehr zur Auswahl." : "Inactive projects are no longer available for selection.",
    "Alle Mitarbeitenden" : "All employees",
    "Nur ausgewählte Mitarbeitende" : "Only selected employees",
    "Mitarbeitende auswählen" : "Select employees",
    "Dieses Projekt ist dem Mitarbeiter nicht zugeordnet." : "This project is not assigned to the employee.",
    "z.B. PRJ-001" : "e.g. PRJ-001",
    "Farbe" : "Colour",
    "Projekt erstellt" : "Project created",
    "Projekt aktualisiert" : "Project updated",
    "Projekt gelöscht" : "Project deleted",
    "Keine Projekte" : "No Projects",
    "Legen Sie Projekte an, um projektbezogene Zeiterfassung zu nutzen." : "Create projects to use project-based time tracking.",
    "Projekt löschen?" : "Delete project?",
    "Möchten Sie das Projekt \"{name}\" wirklich löschen?" : "Do you really want to delete the project \"{name}\"?",
    "Kein Projekt" : "No Project",
    "Bemerkung" : "Note",
    "Beschreibung" : "Description",
    "Monat prüfen" : "Review month",
    "Keine Einträge in diesem Monat." : "No entries in this month.",
    "Min" : "min",
    "Speichern" : "Save",
    "Abbrechen" : "Cancel",
    "Löschen" : "Delete",
    "Bearbeiten" : "Edit",
    "Erstellen" : "Create",
    "Zurücksetzen" : "Reset",
    "Standard-Arbeitszeiten" : "Default Working Hours",
    "Diese Zeiten werden beim Anlegen neuer Zeiteinträge vorausgefüllt." : "These times will be pre-filled when creating new time entries.",
    "Leer lassen für Standardwerte (08:00 - 17:00)." : "Leave empty for defaults (08:00 - 17:00).",
    "z.B. 08:00" : "e.g. 08:00",
    "z.B. 17:00" : "e.g. 17:00",
    "Mindestpause: {minutes} min (§4 ArbZG)" : "Minimum break: {minutes} min (§4 German Working Hours Act)",
    "Maximale tägliche Arbeitszeit (Stunden)" : "Maximum daily working time (hours)",
    "Max. {hours} Std./Tag" : "Max. {hours} hrs/day",
    "Nach §3 ArbZG sind maximal 10 Stunden erlaubt (Ausnahmen möglich)." : "According to §3 of the German Working Hours Act, a maximum of 10 hours is permitted (exceptions possible).",
    "Keine Einträge" : "No Entries",
    "Eintrag gelöscht" : "Entry deleted",
    "Eintrag löschen" : "Delete entry",
    "Möchten Sie diesen Eintrag wirklich löschen?" : "Do you really want to delete this entry?",
    "Status" : "Status",
    "Entwurf" : "Draft",
    "Storniert" : "Cancelled",
    "Ausstehend" : "Pending",
    "Beantragt" : "Requested",
    "Erneute Genehmigung erforderlich" : "Re-approval required",
    "Einreichen" : "Submit",
    "Genehmigen" : "Approve",
    "Ablehnen" : "Reject",
    "Stornieren" : "Cancel",
    "Monat einreichen" : "Submit Month",
    "{count} Einträge wurden eingereicht." : "{count} entries have been submitted.",
    "{count} Einträge genehmigt" : "{count} entries approved",
    "Feiertage" : "Public Holidays",
    "Einträge" : "Entries",
    "Soll" : "Target",
    "Ist" : "Actual",
    "Überstunden" : "Overtime",
    "Minusstunden" : "Deficit Hours",
    "Monat" : "Month",
    "Keine Daten für dieses Jahr vorhanden." : "No data available for this year.",
    "Neue Abwesenheit" : "New Absence",
    "Abwesenheit löschen" : "Delete absence",
    "Abwesenheit stornieren" : "Cancel absence",
    "Abwesenheit erstellt" : "Absence created",
    "Abwesenheit aktualisiert" : "Absence updated",
    "Abwesenheit gelöscht" : "Absence deleted",
    "Abwesenheit genehmigt" : "Absence approved",
    "Abwesenheit abgelehnt" : "Absence rejected",
    "Abwesenheit storniert" : "Absence cancelled",
    "Anspruch" : "Entitlement",
    "Gesamt" : "Total",
    "Genommen" : "Used",
    "Tage" : "Days",
    "Zeitraum" : "Period",
    "Offen" : "Open",
    "Genehmigt am" : "Approved on",
    "Keine genehmigten Monate" : "No approved months",
    "Der Monat wird wieder zur Bearbeitung freigegeben. Der Mitarbeiter kann korrigieren und erneut einreichen." : "The month is unlocked for editing again. The employee can correct it and resubmit.",
    "Art" : "Type",
    "Umfang" : "Scope",
    "Ganzer Tag" : "Full Day",
    "Halber Tag" : "Half Day",
    "½ Tag" : "½ day",
    "1 Tag" : "1 day",
    "Von" : "From",
    "Bis" : "To",
    "Aktionen" : "Actions",
    "Urlaub" : "Vacation",
    "Krankheit" : "Sick Leave",
    "Kind krank" : "Child Sick",
    "Sonderurlaub" : "Special Leave",
    "Unbezahlter Urlaub" : "Unpaid Leave",
    "Fortbildung" : "Training",
    "Keine Abwesenheiten" : "No Absences",
    "Sie haben noch keine Abwesenheiten eingetragen." : "You haven't recorded any absences yet.",
    "Kein Team" : "No Team",
    "Sie haben keine Teammitglieder." : "You have no team members.",
    "Std./Woche" : "hrs/week",
    "Woche" : "Week",
    "Stunden" : "Hours",
    "Neuer Mitarbeiter" : "New Employee",
    "Mitarbeiter bearbeiten" : "Edit Employee",
    "Mitarbeiter" : "Employee",
    "Nextcloud-Benutzer" : "Nextcloud User",
    "Benutzer auswählen" : "Select user",
    "Vorname" : "First Name",
    "Nachname" : "Last Name",
    "E-Mail" : "Email",
    "Name" : "Name",
    "Personalnummer" : "Staff Number",
    "Personalnr." : "Staff No.",
    "Wochenstunden" : "Weekly Hours",
    "Wochenstd." : "Hrs/week",
    "Wochenstunden (berechnet)" : "Weekly Hours (calculated)",
    "Arbeitstage pro Woche" : "Working Days per Week",
    "Vorgesetzter" : "Supervisor",
    "Kein Vorgesetzter" : "No Supervisor",
    "Bundesland" : "Federal State",
    "Eintrittsdatum" : "Start Date",
    "Austrittsdatum" : "End Date",
    "Aktiv" : "Active",
    "Inaktiv" : "Inactive",
    "Urlaubstage" : "Vacation Days",
    "Keine Mitarbeiter" : "No Employees",
    "Legen Sie Mitarbeiter an, um die Zeiterfassung zu nutzen." : "Add employees to start using time tracking.",
    "Mitarbeiter erstellt" : "Employee created",
    "Mitarbeiter aktualisiert" : "Employee updated",
    "Mitarbeiter gelöscht" : "Employee deleted",
    "Mitarbeiter löschen?" : "Delete employee?",
    "Möchten Sie den Mitarbeiter \"{name}\" wirklich löschen?" : "Do you really want to delete the employee \"{name}\"?",
    "Diese Aktion kann nicht rückgängig gemacht werden." : "This action cannot be undone.",
    "Arbeitszeitprofile" : "Work Schedules",
    "Neues Profil" : "New Schedule",
    "Neues Profil anlegen" : "Create New Schedule",
    "Profil bearbeiten" : "Edit Schedule",
    "Profil löschen?" : "Delete schedule?",
    "Möchten Sie dieses Arbeitszeitprofil wirklich löschen?" : "Do you really want to delete this work schedule?",
    "Gültig ab" : "Valid from",
    "Mo" : "Mon",
    "Di" : "Tue",
    "Mi" : "Wed",
    "Do" : "Thu",
    "Fr" : "Fri",
    "Sa" : "Sat",
    "So" : "Sun",
    "Berechtigungen" : "Permissions",
    "HR-Manager" : "HR Manager",
    "Berechnung anzeigen" : "Show calculation",
    "Soll-Berechnung" : "Target calculation",
    "Ist-Berechnung" : "Actual calculation",
    "Arbeitstage ({count})" : "Working days ({count})",
    "davon Feiertage ({count})" : "thereof public holidays ({count})",
    "Bezahlte Abwesenheiten ({count} Tage)" : "Paid absences ({count} days)",
    "Soll-Reduktion ({count} Tage)" : "Target reduction ({count} days)",
    "Geleistete Arbeitszeit" : "Hours worked",
    "Benutzer oder Gruppen auswählen" : "Select users or groups",
    "HR-Manager gespeichert" : "HR managers saved",
    "Firmendaten" : "Company Data",
    "Firmenname" : "Company Name",
    "Standard-Bundesland" : "Default Federal State",
    "Arbeitszeit-Regeln" : "Working Time Rules",
    "Projekt erforderlich" : "Project Required",
    "Beschreibung erforderlich" : "Description Required",
    "Zukünftige Einträge erlauben" : "Allow Future Entries",
    "Genehmigung erforderlich" : "Approval Required",
    "Pausenregelung (§4 ArbZG)" : "Break Rules (§4 German Working Hours Act)",
    "Mindestpause gemäß deutschem Arbeitszeitgesetz" : "Minimum break according to the German Working Hours Act",
    "Bei >6h Arbeitszeit (min)" : "For >6h working time (min)",
    "Bei >9h Arbeitszeit (min)" : "For >9h working time (min)",
    "Sondertage" : "Special Days",
    "Feiertage verwalten" : "Manage Public Holidays",
    "Feiertage anzeigen, hinzufügen, bearbeiten und löschen." : "View, add, edit and delete public holidays.",
    "Neuer Feiertag" : "New Public Holiday",
    "Feiertag bearbeiten" : "Edit Public Holiday",
    "Feiertag hinzufügen" : "Add Public Holiday",
    "Feiertag erstellt" : "Public holiday created",
    "Feiertag löschen" : "Delete public holiday",
    "Möchten Sie den Feiertag \"{name}\" ({count} Bundesländer) wirklich löschen?" : "Do you really want to delete the public holiday \"{name}\" ({count} federal states)?",
    "Keine Feiertage" : "No Public Holidays",
    "Keine Feiertage für diese Auswahl gefunden." : "No public holidays found for this selection.",
    "Hinweis: Änderungen wirken sich auf neu generierte Feiertage aus. Generieren Sie die Feiertage erneut, um die Änderungen anzuwenden." : "Note: Changes affect newly generated public holidays. Regenerate public holidays to apply the changes.",
    "{count} Feiertag(e) aktualisiert" : "{count} public holiday(s) updated",
    "{count} Feiertag(e) gelöscht" : "{count} public holiday(s) deleted",
    "z.B. Brückentag" : "e.g. Bridge Day",
    "Jahr" : "Year",
    "Alle Bundesländer" : "All Federal States",
    "Bundesländer" : "Federal States",
    "Alle auswählen" : "Select all",
    "Alle abwählen" : "Deselect all",
    "Auto" : "Auto",
    "Auto-Generieren" : "Auto-Generate",
    "Manuell" : "Manual",
    "{count} Feiertage für {year} generiert" : "{count} public holidays generated for {year}",
    "Genehmigte Monatsberichte werden automatisch als PDF archiviert." : "Approved monthly reports are automatically archived as PDF.",
    "Archiv-Ordner" : "Archive Folder",
    "Archiv-Ordner auswählen" : "Select archive folder",
    "Ordner auswählen" : "Select folder",
    "Nicht konfiguriert" : "Not configured",
    "PDFs werden in Ihrem persönlichen Ordner gespeichert. Nur Sie haben Zugriff." : "PDFs are saved in your personal folder. Only you have access.",
    "Struktur: {path}/{Jahr}/{Nachname_Vorname}/Arbeitszeitnachweis_YYYY-MM.pdf" : "Structure: {path}/{Year}/{Lastname_Firstname}/Timesheet_YYYY-MM.pdf",
    "Einstellung gespeichert" : "Setting saved",
    "Fehler beim Speichern" : "Error saving",
    "Fehler beim Speichern des Profils" : "Error saving schedule",
    "Fehler beim Löschen" : "Error deleting",
    "Fehler beim Löschen des Profils" : "Error deleting schedule",
    "Fehler beim Erstellen" : "Error creating",
    "Fehler beim Genehmigen" : "Error approving",
    "Fehler beim Einreichen des Monats." : "Error submitting the month.",
    "Fehler beim Ablehnen" : "Error rejecting",
    "Vorheriger Monat" : "Previous month",
    "Nächster Monat" : "Next month",
    "Vorheriges Jahr" : "Previous year",
    "Nächstes Jahr" : "Next year",
    "Bayern" : "Bavaria",
    "Baden-Württemberg" : "Baden-Württemberg",
    "Berlin" : "Berlin",
    "Brandenburg" : "Brandenburg",
    "Bremen" : "Bremen",
    "Hamburg" : "Hamburg",
    "Hessen" : "Hesse",
    "Mecklenburg-Vorpommern" : "Mecklenburg-Western Pomerania",
    "Niedersachsen" : "Lower Saxony",
    "Nordrhein-Westfalen" : "North Rhine-Westphalia",
    "Rheinland-Pfalz" : "Rhineland-Palatinate",
    "Saarland" : "Saarland",
    "Sachsen" : "Saxony",
    "Sachsen-Anhalt" : "Saxony-Anhalt",
    "Schleswig-Holstein" : "Schleswig-Holstein",
    "Thüringen" : "Thuringia",
    "Januar" : "January",
    "Februar" : "February",
    "März" : "March",
    "April" : "April",
    "Mai" : "May",
    "Juni" : "June",
    "Juli" : "July",
    "August" : "August",
    "September" : "September",
    "Oktober" : "October",
    "November" : "November",
    "Dezember" : "December",
    "Jan" : "Jan",
    "Feb" : "Feb",
    "Mrz" : "Mar",
    "Apr" : "Apr",
    "Jun" : "Jun",
    "Jul" : "Jul",
    "Aug" : "Aug",
    "Sep" : "Sep",
    "Okt" : "Oct",
    "Nov" : "Nov",
    "Dez" : "Dec",
    "Std." : "hrs",
    "Abwesend" : "Absent",
    "Mitarbeiter ist abwesend. Grund ist nur für Vorgesetzte sichtbar." : "Employee is absent. Reason is only visible to supervisors.",
    "Krank" : "Sick",
    "Freizeitausgleich" : "Compensatory Leave",
    "Alle Mitarbeiter" : "All Employees",
    "Mein Team" : "My Team",
    "Keine Mitarbeiter sichtbar" : "No employees visible",
    "Detailgrad" : "Detail Level",
    "Nur „Abwesend\" anzeigen" : "Only show \"Absent\"",
    "Grund anzeigen (Urlaub, Fortbildung, ...)" : "Show reason (vacation, training, ...)",
    "Abwesenheiten sichtbar für" : "Absences visible for",
    "Niemand" : "Nobody",
    "Datenschutz" : "Privacy",
    "Legen Sie fest, wer Ihre Abwesenheiten in der Abwesenheitsübersicht sehen kann. Vorgesetzte und HR sehen Ihre Abwesenheiten immer." : "Define who can see your absences in the absence overview. Supervisors and HR can always see your absences.",
    "Monatsbericht als PDF" : "Monthly Report as PDF",
    "Diese Abwesenheit löschen?" : "Delete this absence?",
    "Diese Abwesenheit stornieren? Sie bleibt mit Status \"Storniert\" sichtbar." : "Cancel this absence? It will remain visible with status \"Cancelled\".",
    "Gespeichert" : "Saved",
    "Entfernen" : "Remove",
    "Fehler beim Entfernen" : "Error removing",
    "Zur Kenntnisnahme" : "For information",
    "Abwesenheitstypen" : "Absence Types",
    "Bezahlter Erholungsurlaub. Wird vom Urlaubskonto abgezogen." : "Paid vacation. Deducted from your vacation allowance.",
    "Krankmeldung. Arbeitszeit gilt als geleistet, keine Urlaubstage." : "Sick note. Counts as hours worked, no vacation days deducted.",
    "Ihr Kind ist krank. Wie Krankheit, keine Urlaubstage." : "Your child is sick. Same as sickness, no vacation days.",
    "Bezahlte Freistellung, z.B. Hochzeit, Umzug oder Trauerfall." : "Paid leave, e.g. wedding, moving or bereavement.",
    "Schulung, Seminar oder Konferenz. Zählt als Arbeitszeit." : "Training, seminar or conference. Counts as working hours.",
    "Freistellung ohne Gehalt. Reduziert die Soll-Stunden." : "Leave without pay. Reduces your target hours.",
    "Überstunden als Freizeit nehmen. Reduziert die Überstunden." : "Use overtime as time off. Reduces your overtime.",
    "Freizeitausgleich ({count} Tage, nicht gutgeschrieben)" : "Compensatory time ({count} days, not credited)",
    "Freizeitausgleich wird nicht als Arbeitszeit gutgeschrieben. Das Soll bleibt bestehen, dadurch sinken die Überstunden." : "Compensatory time is not credited as working time. The target stays the same, which lowers your overtime.",
    "Differenz zwischen Soll und Ist. Im laufenden Monat kann sich der Wert noch ändern." : "Difference between target and actual. May still change during the current month.",
    "Das Soll wird anteilig bis gestern berechnet. Der heutige Tag zählt erst mit, sobald du Zeit erfasst." : "The target is calculated proportionally up to yesterday. Today only counts once you record time.",
    "Gesetzliche Pausenregelung: Ab 6 Stunden Arbeitszeit mindestens 30 Minuten, ab 9 Stunden mindestens 45 Minuten Pause." : "Legal break regulation: At least 30 minutes break for more than 6 hours of work, at least 45 minutes for more than 9 hours.",
    "Neue Mitarbeiter bekommen dieses Bundesland automatisch zugewiesen. Jeder Mitarbeiter kann ein eigenes Bundesland haben — das bestimmt, welche Feiertage für ihn gelten." : "New employees are automatically assigned this federal state. Each employee can have their own — it determines which public holidays apply.",
    "Neue Mitarbeiter bekommen diese Wochenstunden voreingestellt. Sie können im Mitarbeiterprofil individuell angepasst werden." : "New employees get these weekly hours as default. Can be adjusted individually in the employee profile.",
    "Neue Mitarbeiter bekommen diesen Urlaubsanspruch voreingestellt. Der tatsächliche Anspruch wird im Mitarbeiterprofil festgelegt." : "New employees get this vacation entitlement as default. The actual entitlement is set in the employee profile.",
    "Wenn ein Zeiteintrag diesen Wert überschreitet, wird eine Warnung angezeigt. Nach §3 ArbZG sind maximal 10 Stunden erlaubt." : "A warning is shown when a time entry exceeds this value. German law (§3 ArbZG) allows a maximum of 10 hours.",
    "Wenn aktiv, muss bei jedem Zeiteintrag ein Projekt ausgewählt werden — sonst lässt sich der Eintrag nicht speichern." : "When active, a project must be selected for every time entry — otherwise it cannot be saved.",
    "Wenn aktiv, muss bei jedem Zeiteintrag eine Beschreibung eingetragen werden — sonst lässt sich der Eintrag nicht speichern." : "When active, a description must be entered for every time entry — otherwise it cannot be saved.",
    "Wenn deaktiviert, können Mitarbeiter nur für heute oder vergangene Tage Zeiten eintragen — nicht im Voraus." : "When disabled, employees can only enter times for today or past days — not in advance.",
    "Wenn aktiv, durchlaufen Zeiteinträge einen Freigabe-Workflow: Mitarbeitende reichen den Monat ein, Vorgesetzte genehmigen ihn. Ist die Option deaktiviert, entfällt dieser Schritt und die erfassten Zeiten gelten direkt. Die Stundenberechnung ist in beiden Fällen gleich." : "When active, time entries go through an approval workflow: employees submit the month and supervisors approve it. If disabled, this step is skipped and recorded times apply directly. The hours calculation is the same either way.",
    "Gesetzliche Mindestpause bei mehr als 6 Stunden Arbeitszeit. Wird beim Anlegen eines Zeiteintrags automatisch als Vorschlag eingetragen." : "Legal minimum break for more than 6 hours of work. Automatically suggested when creating a time entry.",
    "Gesetzliche Mindestpause bei mehr als 9 Stunden Arbeitszeit. Wird beim Anlegen eines Zeiteintrags automatisch als Vorschlag eingetragen." : "Legal minimum break for more than 9 hours of work. Automatically suggested when creating a time entry.",
    "Wenn ein Monat genehmigt wird, speichert WorkTime automatisch einen PDF-Bericht in diesem Ordner. Der Ordner liegt in Ihrem persönlichen Speicher — nur Sie als Admin haben Zugriff. Die automatische Archivierung greift nur bei aktivierter Genehmigung; ist sie deaktiviert, nutzen Sie den PDF-Export in der Monatsübersicht." : "When a month is approved, WorkTime automatically saves a PDF report in this folder. The folder is in your personal storage — only you as admin have access. Automatic archiving only applies when approval is enabled; if disabled, use the PDF export in the monthly overview.",
    "Vertraglich vereinbarte Arbeitszeit pro Woche. Daraus berechnet WorkTime das tägliche Soll (Wochenstunden ÷ Arbeitstage pro Woche)." : "Contractually agreed working hours per week. WorkTime calculates the daily target from this (weekly hours ÷ working days per week).",
    "Jährlicher Urlaubsanspruch. Jeder genommene Urlaubstag wird davon abgezogen. Der Resturlaub wird in der Zeiterfassung angezeigt." : "Annual vacation entitlement. Each taken vacation day is deducted. The remaining entitlement is shown in the time tracking.",
    "An wie vielen Tagen pro Woche wird gearbeitet? Daraus und aus den Wochenstunden ergibt sich das tägliche Soll. Beispiel: 40 Std. auf 5 Tage = 8 Std./Tag, 30 Std. auf 4 Tage = 7,5 Std./Tag." : "How many days per week does this employee work? Together with weekly hours, this determines the daily target. Example: 40 hrs over 5 days = 8 hrs/day, 30 hrs over 4 days = 7.5 hrs/day.",
    "Legt fest, welche gesetzlichen Feiertage für diesen Mitarbeiter gelten. Bayern hat z.B. mehr Feiertage als Hamburg." : "Determines which public holidays apply to this employee. Bavaria has more public holidays than Hamburg, for example.",
    "Diese Person kann die Zeiteinträge und Abwesenheitsanträge dieses Mitarbeiters einsehen und genehmigen." : "This person can view and approve this employee's time entries and absence requests.",
    "Ab diesem Datum erscheint der Mitarbeiter in der Zeiterfassung. Für Monate davor werden keine Sollstunden berechnet." : "The employee appears in time tracking from this date. No target hours are calculated for months before this.",
    "Ab diesem Datum kann der Mitarbeiter keine neuen Einträge mehr erfassen. Alle bisherigen Daten bleiben erhalten." : "From this date, the employee can no longer create new entries. All existing data is preserved.",
    "Diese Abwesenheiten (z.B. Krankheit) werden nur gemeldet und brauchen keine Genehmigung. Sie werden automatisch in der Sollberechnung berücksichtigt." : "These absences (e.g. sick leave) are only reported and don't need approval. They are automatically included in target calculations.",
    "Legt fest, ob Kollegen in der Abwesenheitsübersicht Ihren Abwesenheitsgrund sehen (z.B. Urlaub) oder nur Abwesend. Vorgesetzte und HR sehen immer den Grund." : "Determines whether colleagues see your absence reason (e.g. vacation) or just 'absent' in the absence overview. Supervisors and HR always see the reason.",
    "Admin: Volle Rechte (automatisch). HR-Manager: Mitarbeiter verwalten und Anträge genehmigen (manuell zuweisen). Vorgesetzter: Genehmigt Zeiten seines Teams (automatisch). Mitarbeiter: Eigene Zeiten erfassen (automatisch)." : "Admin: Full rights (automatic). HR Manager: Manage employees and approve requests (assign manually). Supervisor: Approves their team's time entries (automatic). Employee: Track own time (automatic).",
    "Jahresübertrag" : "Yearly Carryover",
    "Überstunden und Resturlaub aus dem Vorjahr händisch übertragen. Durchgeführte Überträge sind verbindlich und unveränderbar." : "Manually transfer overtime and remaining vacation from the previous year. Completed carryovers are binding and immutable.",
    "Alle Monate {year} genehmigt" : "All months {year} approved",
    "{year}: Noch offene Monate" : "{year}: Open months remaining",
    "Übertrag" : "Carryover",
    "Übertrag durchführen" : "Complete carryover",
    "Durchgeführt" : "Completed",
    "Korrektur" : "Correction",
    "Übertrag durchgeführt" : "Carryover completed",
    "Übertrag korrigiert" : "Carryover corrected",
    "Übertrag korrigieren" : "Correct carryover",
    "Übertrag korrigieren für {name}" : "Correct carryover for {name}",
    "Der bestehende Übertrag wird storniert und durch die neuen Werte ersetzt. Beide Einträge bleiben im Audit-Log erhalten." : "The existing carryover will be cancelled and replaced with new values. Both entries are preserved in the audit log.",
    "Neue Überstunden (Std.)" : "New overtime (hrs)",
    "Neuer Resturlaub (Tage)" : "New remaining vacation (days)",
    "Begründung (Pflicht)" : "Reason (required)",
    "Grund für die Korrektur" : "Reason for correction",
    "Stornieren & Ersetzen" : "Cancel & Replace",
    "Übertrag Vorjahr" : "Previous Year Carryover",
    "%1$s hat eine Abwesenheit (%2$s, %3$s - %4$s) zur Genehmigung eingereicht" : "%1$s submitted an absence (%2$s, %3$s - %4$s) for approval",
    "Deine Abwesenheit (%1$s, %2$s - %3$s) wurde genehmigt" : "Your absence (%1$s, %2$s - %3$s) has been approved",
    "Deine Abwesenheit (%1$s, %2$s - %3$s) wurde abgelehnt" : "Your absence (%1$s, %2$s - %3$s) has been rejected",
    "Information: %1$s ist abwesend (%2$s, %3$s - %4$s)" : "Information: %1$s is absent (%2$s, %3$s - %4$s)",
    "%1$s hat Abwesenheit (%2$s, %3$s - %4$s) storniert" : "%1$s cancelled absence (%2$s, %3$s - %4$s)",
    "%1$s hat Zeiteinträge für %2$s zur Genehmigung eingereicht" : "%1$s submitted time entries for %2$s for approval",
    "Deine Zeiteinträge für %s wurden genehmigt" : "Your time entries for %s have been approved",
    "Deine Zeiteinträge für %s wurden abgelehnt" : "Your time entries for %s have been rejected",
    "Die Genehmigung deiner Zeiteinträge für %s wurde zurückgenommen. Bitte erneut einreichen." : "The approval of your time entries for %s has been revoked. Please submit again.",
    "Zukünftige Einträge sind nicht erlaubt" : "Future entries are not allowed",
    "Ungültiges Zeitformat" : "Invalid time format",
    "Mindestpause nicht eingehalten: Bei %1$d Min Arbeitszeit sind %2$d Min Pause erforderlich (§4 ArbZG), erfasst sind %3$d Min (Lücken zwischen Einträgen zählen als Pause)." : "Minimum break not met: %1$d min of working time require %2$d min break (§4 German Working Hours Act); %3$d min recorded (gaps between entries count as break).",
    "Maximale tägliche Arbeitszeit (%1$s Std.) überschritten: an diesem Tag sind %2$s Std. erfasst." : "Maximum daily working time (%1$s hours) exceeded: %2$s hours recorded on this day.",
    "Pause kann nicht negativ sein" : "Break cannot be negative",
    "An diesem Tag haben Sie %s. Bitte stornieren Sie zuerst die Abwesenheit." : "You have %s on this day. Please cancel the absence first.",
    "An folgenden Tagen sind bereits Zeiteinträge erfasst: %s. Eine ganztägige Abwesenheit ist dort nicht möglich. Bitte zuerst die Zeiteinträge entfernen." : "Time entries already exist on the following days: %s. A full-day absence is not possible there. Please remove the time entries first.",
    "Überlappung mit bestehendem Eintrag (%1$s - %2$s)" : "Overlap with existing entry (%1$s - %2$s)",
    "Scope muss zwischen 0 und 1 liegen" : "Scope must be between 0 and 1",
    "Halber Tag ist nur für einen einzelnen Tag möglich" : "Half day is only possible for a single day",
    "Halbtägige Abwesenheit (%s) und Zeiteintrag am selben Tag." : "Half-day absence (%s) and time entry on the same day.",
    "Ein Profil mit diesem Gültig-ab Datum existiert bereits" : "A profile with this valid-from date already exists",
    "Maximale tägliche Arbeitszeit ist %s Stunden (siehe Einstellungen)" : "Maximum daily working time is %s hours (see settings)",
    "Begründung erforderlich" : "Reason required",
    "Steuert firmenweit, ob erfasste Zeiten durch Vorgesetzte freigegeben werden müssen. Diese Einstellung betrifft alle Mitarbeitenden." : "Controls company-wide whether recorded times must be approved by supervisors. This setting affects all employees.",
    "Genehmigung aktivieren" : "Enable approval",
    "Genehmigung deaktivieren" : "Disable approval",
    "Ab jetzt müssen Zeiten eingereicht und durch Vorgesetzte freigegeben werden." : "From now on, times must be submitted and approved by supervisors.",
    "Der Freigabe-Schritt entfällt für alle Mitarbeitenden. Erfasste Zeiten gelten dann direkt. Bereits genehmigte Einträge bleiben gesperrt und können im Aus-Modus nicht mehr aufgemacht werden." : "The approval step is removed for all employees. Recorded times then apply directly. Already approved entries remain locked and cannot be reopened while approval is off.",
    "Fortfahren" : "Continue",
    "HR-Manager entfernen" : "Remove HR Manager",
    "{names} verliert damit die HR-Manager-Rechte (Mitarbeiter verwalten, Zeiten genehmigen). Fortfahren?" : "{names} will lose HR manager rights (manage employees, approve times). Continue?",
    "{names} verlieren damit die HR-Manager-Rechte (Mitarbeiter verwalten, Zeiten genehmigen). Fortfahren?" : "{names} will lose HR manager rights (manage employees, approve times). Continue?",
    "Feiertage neu erstellen" : "Regenerate public holidays",
    "Die automatisch erzeugten Feiertage für {year} werden für alle Bundesländer neu erstellt. Manuell angelegte Feiertage bleiben erhalten." : "The automatically generated public holidays for {year} are recreated for all federal states. Manually created holidays are preserved.",
    "Projekte" : "Projects",
    "Genehmigung" : "Approval",
    "Pausenregelung" : "Break Rules",
    "PDF-Archiv" : "PDF Archive",
    "Heute" : "Today",
    "Jetzt" : "Now",
    "noch {hours} h bis Jahressoll" : "{hours} h to annual target",
    "Klick auf einen Monat öffnet ihn in der Liste-Ansicht." : "Click a month to open it in the list view.",
    "Gesamt bis heute" : "Total to date",
    "PDF Monatsbericht" : "PDF monthly report",
    "Weitere Aktionen" : "More actions",
    "Ist / Soll · {year}" : "Actual / Target · {year}",
    "Ist / Soll · {month}" : "Actual / Target · {month}",
    "Freizeitausgleich reduziert die Überstunden automatisch." : "Compensatory leave automatically reduces overtime.",
    "Hinweis: ca. {requested} Werktage im Zeitraum (Resturlaub: {available}). Abgezogen werden nur Arbeitstage laut Arbeitszeitmodell." : "Note: approx. {requested} working days in the period (remaining: {available}). Only working days according to the work schedule are deducted.",
    "Dieser Zeitraum ist abgeschlossen. Bitte wende dich an HR." : "This period is closed. Please contact HR.",
    "Begründung erforderlich (mindestens 10 Zeichen)." : "A reason is required (at least 10 characters).",
    "Korrigieren" : "Correct",
    "Zeiten/Abwesenheiten dieses Mitarbeiters korrigieren" : "Correct this employee's times/absences",
    "Korrektur-Modus · {name}" : "Correction mode · {name}",
    "Änderungen werden protokolliert und dem Mitarbeiter angezeigt." : "Changes are logged and shown to the employee.",
    "Korrektur beenden" : "End correction",
    "Korrektur – Begründung" : "Correction – reason",
    "Begründung für die Korrektur" : "Reason for the correction",
    "Die Änderung wird protokolliert. In abgeschlossenen Monaten ist eine Begründung Pflicht; der Monat wird danach zur erneuten Genehmigung geöffnet." : "The change is logged. In closed months a reason is mandatory; the month is then reopened for re-approval.",
    "Begründung (mindestens 10 Zeichen)" : "Reason (at least 10 characters)",
    "z. B. Stempelfehler nach Rücksprache mit der Mitarbeiterin korrigiert" : "e.g. corrected a clock-in error after consulting the employee",
    "Korrektur speichern" : "Save correction",
    "Die Genehmigung deiner Zeiteinträge für %1$s wurde zurückgenommen (Grund: %2$s). Bitte erneut einreichen." : "The approval of your time entries for %1$s was withdrawn (reason: %2$s). Please resubmit.",
    "Ausgezahlt {year}" : "Paid out {year}",
    "Überstunden-Auszahlung" : "Overtime payout",
    "Überstunden in Geld vergüten und vom Saldo abziehen. Auszahlungen werden im Audit-Log protokolliert." : "Compensate overtime with money and deduct it from the balance. Payouts are recorded in the audit log.",
    "Saldo aktuell" : "Current balance",
    "Auszahlen" : "Pay out",
    "Auszahlungs-Historie {year}" : "Payout history {year}",
    "Notiz" : "Note",
    "Auszahlung löschen" : "Delete payout",
    "Noch keine Auszahlungen erfasst." : "No payouts recorded yet.",
    "Überstunden auszahlen" : "Pay out overtime",
    "Überstunden auszahlen für {name}" : "Pay out overtime for {name}",
    "Verfügbarer Saldo: {saldo} h" : "Available balance: {saldo} h",
    "Auszuzahlende Stunden" : "Hours to pay out",
    "z. B. 8:00 oder 8,5" : "e.g. 8:00 or 8.5",
    "Notiz / Grund (Pflicht)" : "Note / reason (required)",
    "mind. 10 Zeichen, landet im Audit-Log" : "at least 10 characters, stored in the audit log",
    "Auszahlung erfassen" : "Record payout",
    "Konten" : "Accounts",
    "Bitte gültige Stunden eingeben." : "Please enter valid hours.",
    "Die Auszahlung darf den verfügbaren Saldo nicht überschreiten." : "The payout must not exceed the available balance.",
    "Bitte einen Grund mit mindestens 10 Zeichen angeben." : "Please provide a reason with at least 10 characters.",
    "Auszahlung erfasst" : "Payout recorded",
    "Diese Auszahlung löschen? Der Überstundensaldo wird wieder erhöht." : "Delete this payout? The overtime balance will increase again.",
    "Auszahlung gelöscht" : "Payout deleted",
    "Für wen" : "For whom",
    "Alle aktiven Mitarbeiter" : "All active employees",
    "Ausgewählte Mitarbeiter" : "Selected employees",
    "z. B. Betriebsferien Weihnachten" : "e.g. Christmas company holidays",
    "Betriebsferien eintragen" : "Add company holidays",
    "{count} Mitarbeiter eingetragen." : "{count} employees added.",
    "Nicht gebucht – bitte einzeln klären:" : "Not booked – please handle individually:",
    "Eingetragene Betriebsferien" : "Company holidays entered",
    "Noch keine Betriebsferien eingetragen." : "No company holidays entered yet.",
    "nicht genug Resturlaub" : "not enough remaining vacation",
    "Zeiteinträge im Zeitraum vorhanden" : "time entries exist in the period",
    "Betriebsferien eingetragen." : "Company holidays added.",
    "Fehler beim Eintragen" : "Error while saving",
    "Betriebsferien entfernt." : "Company holidays removed.",
    "Betriebsferien" : "Company holidays",
    "Tragen Sie Betriebsferien zentral für alle oder ausgewählte Mitarbeiter als Urlaub ein." : "Enter company holidays centrally as vacation for all or selected employees.",
    "{count} ausgewählt" : "{count} selected",
    "Kein Mitarbeiter gefunden." : "No employee found.",
    "Betriebsschließung" : "Company closure",
    "Bezahlte Freistellung bei Betriebsferien. Kein Urlaubs- oder Überstundenabzug." : "Paid leave during company vacation. No vacation or overtime deduction.",
    "Wenn der Resturlaub nicht reicht" : "If the remaining vacation is not enough",
    "Resturlaub reichte nicht – aufgeteilt gebucht:" : "Remaining vacation was not enough – booked split:",
    "Nicht buchen, nur auflisten" : "Do not book, only list",
    "Bezahlte Freistellung (Betriebsschließung)" : "Paid leave (company closure)",
    "Urlaub ins Minus" : "Vacation into negative",
    "{name} – {vacation} Tage Urlaub + {overage} Tage {type}" : "{name} – {vacation} days vacation + {overage} days {type}",
    "Betriebsschließung kann nur zentral über die Betriebsferien gesetzt werden" : "Company closure can only be set centrally via the company vacation settings",
    "Mitarbeiter mit bereits erfassten Zeiten im Zeitraum werden nicht gebucht, sondern Ihnen aufgelistet." : "Employees with time entries already recorded in the period are not booked but listed for you.",
    "Teilweise gebucht – bereits eigene Abwesenheit im Zeitraum:" : "Partially booked – own absence already in the period:",
    "bereits eigene Abwesenheit im gesamten Zeitraum" : "own absence already covers the entire period",
    "{name} – {booked} Tage gebucht, {skipped} Tage bereits abwesend" : "{name} – {booked} days booked, {skipped} days already absent",
    "Gültig-ab darf nicht in einen bereits genehmigten Zeitraum fallen. Frühestens möglich: %s" : "Valid-from must not fall within an already approved period. Earliest possible: %s",
    "Gültig-ab darf nicht vor dem Eintrittsdatum liegen (%s)" : "Valid-from must not be before the entry date (%s)",
    "Jahressaldo {year} · Stand heute" : "Year balance {year} · as of today",
    "Vertretung" : "Deputy",
    "Meine Vertretung bei Genehmigungen" : "My deputy for approvals",
    "Keine Vertretung" : "No deputy",
    "Ruhend" : "Resting",
    "Ruhend setzen" : "Set to resting",
    "Reaktivieren" : "Reactivate",
    "Mitarbeiter ruhend setzen: keine neue Erfassung mehr, Daten bleiben einsehbar" : "Set employee to resting: no new records, data stays visible",
    "Mitarbeiter wieder aktiv setzen" : "Set employee back to active",
    "Mitarbeiter ruhend setzen?" : "Set employee to resting?",
    "\"{name}\" kann dann keine Zeiten und Abwesenheiten mehr erfassen. Bestehende Daten bleiben einsehbar und exportierbar." : "\"{name}\" will no longer be able to record times or absences. Existing data stays visible and exportable.",
    "Grund (optional)" : "Reason (optional)",
    "z. B. Elternzeit, Langzeiterkrankung, ausgeschieden" : "e.g. parental leave, long-term illness, left the company",
    "Auswirkungen werden geprüft …" : "Checking impact …",
    "Vertretung wird gestrichen bei:" : "Deputy assignment will be removed for:",
    "Diese Mitarbeiter haben dann keinen Vorgesetzten mehr:" : "These employees will no longer have a supervisor:",
    "Ihre Genehmigungen laufen dann nur noch über Admin oder Personalverwaltung." : "Their approvals will then only go through admin or HR.",
    "Mitarbeiter ruhend gesetzt" : "Employee set to resting",
    "Mitarbeiter reaktiviert" : "Employee reactivated",
    "Ihr Profil ist ruhend gestellt. Sie können Ihre bisherigen Daten weiterhin einsehen und exportieren, aber keine neuen Zeiten oder Abwesenheiten erfassen." : "Your profile is set to resting. You can still view and export your existing data, but you cannot record new times or absences.",
    "Grund: {reason}" : "Reason: {reason}",
    "Für ruhende Mitarbeiter können keine Zeiten erfasst oder geändert werden." : "Times cannot be recorded or changed for resting employees.",
    "Für ruhende Mitarbeiter können keine Abwesenheiten erfasst oder geändert werden." : "Absences cannot be recorded or changed for resting employees.",
    "Profil ruhend – keine Erfassung möglich." : "Profile resting – recording not possible.",
    "Vor Eintritt verbraucht" : "Used before joining",
    "Nicht genügend Urlaubstage. Verfügbar: %s, beantragt: %s." : "Not enough vacation days. Available: %s, requested: %s.",
    "Enddatum muss nach dem Startdatum liegen" : "End date must be after start date",
    "Ungültige Option für den Umgang mit fehlendem Resturlaub" : "Invalid option for handling insufficient remaining vacation",
    "Ungültige Abwesenheitsart" : "Invalid absence type",
    "Es existiert bereits eine Abwesenheit in diesem Zeitraum" : "An absence already exists in this period",
    "Ungültiges Datum. Erwartet wird das Format JJJJ-MM-TT." : "Invalid date. Expected format is YYYY-MM-DD.",
    "Ungültiger Zeitraum. Monat muss zwischen 1 und 12 liegen, Jahr zwischen 2000 und 2100." : "Invalid period. Month must be between 1 and 12, year between 2000 and 2100.",
    "Start- und Enddatum sind erforderlich" : "Start and end date are required",
    "Genehmigungsdaten konnten nicht vollständig geladen werden. Die Liste ist möglicherweise unvollständig." : "Approval data could not be loaded completely. The list may be incomplete.",
    "Farbe wählen" : "Choose colour",
    "Kennzeichnet das Projekt in Listen und in der Auswertung. Neben der Palette sind auch eigene Farben möglich." : "Identifies the project in lists and in the evaluation. Custom colours are possible alongside the palette.",
    "Umfang wird ermittelt …" : "Determining scope …",
    "Mitgelöscht werden:" : "Deleted along with it:",
    "Zu diesem Mitarbeiter sind keine weiteren Daten erfasst." : "No further data is recorded for this employee.",
    "Diese Aktion kann nicht rückgängig gemacht werden. Für Arbeitszeitnachweise können Aufbewahrungspflichten gelten — prüfen Sie das, bevor Sie löschen." : "This action cannot be undone. Retention obligations may apply to working time records — check this before deleting.",
    "Für ausgeschiedene Mitarbeiter ist meist „Ruhend setzen“ richtig: Die Erfassung wird beendet, die Daten bleiben einsehbar und exportierbar." : "For employees who have left, \"Set to resting\" is usually the right choice: time tracking ends, the data stays visible and exportable.",
    "Zeiteinträge" : "Time entries",
    "Jahresüberträge" : "Annual carryovers",
    "Überstunden-Auszahlungen" : "Overtime payouts",
    "Projektzuordnungen" : "Project assignments",
    "Archiv-Aufträge" : "Archive jobs",
    "Protokolleinträge" : "Audit log entries",
    "Gedacht für Elternzeit, Mutterschutz, Langzeiterkrankung oder Ausscheiden. Der Mitarbeiter bleibt in der Verwaltung sichtbar und sieht seine eigenen Daten weiterhin, kann aber nichts mehr erfassen. Personalverwaltung und Administration können zurückliegende Einträge weiterhin korrigieren. Jederzeit umkehrbar." : "Intended for parental leave, maternity protection, long-term illness or leaving the company. The employee stays visible in the administration and can still see their own data, but can no longer record anything. HR and administration can still correct past entries. Reversible at any time.",
    "Wählen Sie eine Person, die Ihre Genehmigungen für Ihr Team übernimmt, während Sie abwesend sind. Die Vertretung wird automatisch aktiv, sobald für Sie eine Abwesenheit genehmigt ist (z. B. Urlaub oder Krankheit) — ohne genehmigte Abwesenheit greift sie nicht. Einmal festgelegt, gilt sie für alle Ihre Abwesenheiten, bis Sie sie ändern." : "Choose a person who takes over your approvals for your team while you are away. The deputy becomes active automatically as soon as an absence has been approved for you (e.g. holiday or sickness) — without an approved absence it does not take effect. Once set, it applies to all your absences until you change it.",
    "Ein genehmigter Monat ist festgeschrieben, damit Auswertungen und Nachweise stabil bleiben. Personalverwaltung und Administration können ihn dennoch korrigieren: Die Änderung verlangt eine Begründung, wird protokolliert, und der Monat geht anschließend zurück zur erneuten Genehmigung." : "An approved month is locked so that reports and records stay stable. HR and administration can still correct it: the change requires a reason, is logged, and the month then goes back for renewed approval.",
    "Voller Jahresanspruch bei diesem Arbeitsmuster. Bei unterjährigem Wechsel wird der tatsächliche Jahresanspruch zeitanteilig aus allen Profilen des Jahres berechnet." : "Full annual entitlement for this work pattern. On a mid-year change the actual annual entitlement is calculated pro rata from all of the year's profiles.",
    "Vorschlag {days} bei {count} Arbeitstagen (30 × Tage ÷ 5) — anpassbar" : "Suggestion {days} for {count} working days (30 × days ÷ 5) — adjustable",
    "Resturlaub aus vorheriger Beschäftigung übernehmen" : "Carry over remaining leave from previous employment",
    "An: der volle Jahresanspruch gilt, abzüglich der bereits genommenen Tage (interner Wechsel, Umstieg auf diese App). Aus: echte Neueinstellung — nur anteilig für die Monate ab Eintritt (Teilurlaub)." : "On: the full annual entitlement applies, minus the days already taken (internal transfer, switch to this app). Off: genuine new hire — prorated only for the months from the entry date (partial leave).",
    "Davon {year} anderswo bereits gewährt/genommen" : "Of which already granted/taken elsewhere in {year}",
    "Urlaubstage, die im Eintrittsjahr bereits genommen oder ausbezahlt wurden — beim vorherigen Arbeitgeber oder vor der Umstellung auf diese App. Bei Übernahme werden sie vom vollen Anspruch abgezogen, bei Neueinstellung begrenzen sie den anteiligen Anspruch (§ 6). Ab dem Folgejahr gilt wieder der volle Jahresanspruch. Halbe Tage sind möglich." : "Leave days already taken or paid out in the entry year — at the previous employer or before switching to this app. For a takeover they are deducted from the full entitlement; for a new hire they cap the prorated entitlement (§ 6). From the following year the full annual entitlement applies again. Half days are possible.",
    "Es läuft bereits eine Stempelung." : "A punch is already running.",
    "Die Stempelung ist bereits pausiert." : "The punch is already paused.",
    "Es läuft gerade keine Pause." : "No break is currently running.",
    "Es läuft keine Stempelung." : "No punch is running.",
    "Ausstempeln" : "Clock out",
    "Zeiteintrag bestätigen" : "Confirm time entry",
    "Prüfe die Zeiten und bestätige, um den Zeiteintrag anzulegen." : "Check the times and confirm to create the time entry.",
    "Du bist seit {hours} h eingestempelt. Bitte prüfe das Ende, bevor du buchst." : "You have been clocked in for {hours} h. Please check the end before booking.",
    "Start" : "Start",
    "Optionale Notiz" : "Optional note",
    "Nettozeit: {duration}" : "Net time: {duration}",
    "Buchen" : "Book",
    "Buchen fehlgeschlagen." : "Booking failed.",
    "Projekt (optional)" : "Project (optional)",
    "Notiz (optional)" : "Note (optional)",
    "Einstempeln" : "Clock in",
    "In Pause" : "On break",
    "Einstempeln fehlgeschlagen." : "Clock-in failed.",
    "Pause fehlgeschlagen." : "Pausing failed.",
    "Fortsetzen fehlgeschlagen." : "Resuming failed.",
    "Max. Pausendauer (min)" : "Max. break duration (min)",
    "Läuft eine laufende Pause (Stempeluhr) länger als dieser Wert, erhält der Mitarbeiter eine Erinnerung („Bist du noch in der Pause?“)." : "If a running break (stopwatch) lasts longer than this value, the employee gets a reminder (“Are you still on a break?”).",
    "Bist du noch in der Pause? Sie läuft seit über %d Minuten." : "Are you still on a break? It has been running for over %d minutes.",
    "Du bist seit über %d Stunden eingestempelt. Nicht vergessen auszustempeln." : "You have been clocked in for over %d hours. Don't forget to clock out.",
    "Kein Gerätetoken übergeben." : "No device token provided.",
    "Nicht unterstützte Plattform." : "Unsupported platform.",
    "Gerätetoken konnte nicht gespeichert werden." : "Could not save the device token.",
    "Gerätetoken konnte nicht entfernt werden." : "Could not remove the device token.",
    "Stornierte und abgelehnte ausblenden" : "Hide cancelled and rejected",
    "{count} ausgeblendet" : "{count} hidden",
    "Legen Sie fest, wie Heiligabend und Silvester behandelt werden." : "Choose how Christmas Eve and New Year's Eve are treated.",
    "Heiligabend (24.12.)" : "Christmas Eve (Dec 24)",
    "Halber freier Tag halbiert das Tagessoll, ganzer freier Tag setzt es auf null. Beispiel: Bei 8 Std./Tag werden 4 Std. bzw. 0 Std. als Soll angerechnet." : "A half day off halves the daily target, a full day off sets it to zero. Example: with 8 hrs/day, 4 hrs or 0 hrs are counted as target.",
    "Normaler Arbeitstag" : "Regular working day",
    "Halber freier Tag" : "Half day off",
    "Ganzer freier Tag" : "Full day off",
    "Silvester (31.12.)" : "New Year's Eve (Dec 31)",
    "Teiltages-Absenzen" : "Partial-day absences",
    "Stundenweise Krankmeldung erlauben" : "Allow hourly sick leave",
    "Wenn aktiv, können Mitarbeitende an einem Einzeltag stundenweise krank sein und die restliche Zeit arbeiten. Die Krank-Stunden füllen bis zum Tagessoll, ohne künstliche Überstunden." : "When enabled, employees can be sick for part of a single day and work the rest. The sick hours fill up to the daily target, without creating artificial overtime.",
    "Krank-Stunden für diesen Tag (leer = ganzer Tag)" : "Sick hours for this day (empty = full day)",
    "Std. krank" : "hrs sick",
    "Für Notarbeit im Urlaub ist eine Begründung erforderlich." : "A reason is required for emergency work during vacation.",
    "%1$s hat Notarbeit im Urlaub am %2$s erfasst" : "%1$s recorded emergency work during vacation on %2$s",
    "Nur Notarbeit-Einträge können freigegeben werden." : "Only emergency-work entries can be approved.",
    "Notarbeit" : "Emergency work",
    "wartet auf Freigabe" : "awaiting approval",
    "Notarbeit/Bereitschaft im Urlaub" : "Emergency/on-call work during vacation",
    "An diesem genehmigten Urlaubstag hast du außerplanmäßig gearbeitet. Der Urlaub bleibt bestehen, die Zeit zählt als Überstunden. Eine Begründung ist Pflicht, der/die Vorgesetzte wird informiert." : "You worked unexpectedly on this approved vacation day. The vacation remains, the time counts as overtime. A reason is mandatory and your supervisor is notified.",
    "Notarbeit freigegeben" : "Emergency work approved",
    "Steuert firmenweit stundenweise Krankmeldungen und Notarbeit an genehmigten Urlaubstagen. Standardmäßig deaktiviert." : "Controls company-wide hourly sick leave and emergency work on approved vacation days. Disabled by default.",
    "Notarbeit im Urlaub erlauben" : "Allow emergency work during vacation",
    "Wenn aktiv, können Mitarbeitende an einem genehmigten vollen Urlaubstag Notarbeit erfassen. Der Urlaub bleibt bestehen, die Arbeit zählt als Überstunden. Eine Begründung ist Pflicht, der/die Vorgesetzte wird informiert." : "When enabled, employees can record emergency work on an approved full vacation day. The vacation remains and the work counts as overtime. A reason is mandatory and the supervisor is notified.",
    "Notarbeit muss freigegeben werden" : "Emergency work must be approved",
    "Wenn aktiv, zählt eine erfasste Notarbeit erst nach Freigabe durch den/die Vorgesetzte(n) in die Überstunden. Nur relevant, wenn Notarbeit im Urlaub erlaubt ist." : "When enabled, recorded emergency work counts toward overtime only after approval by a supervisor. Only relevant when emergency work during vacation is allowed.",
    "Der Monat ist bereits abgeschlossen. Bitte zuerst zur Korrektur freigeben, dann die Notarbeit genehmigen." : "The month is already closed. Please reopen it for correction first, then approve the emergency work.",
    "An diesem Tag haben Sie genehmigten Urlaub. Diese Zeit wird als Notarbeit im Urlaub gebucht (zusätzlich zum Urlaub). Bitte geben Sie eine Begründung an." : "You have approved vacation on this day. This time is booked as emergency work during vacation (in addition to the vacation). Please provide a reason.",
    "Begründung (erforderlich)" : "Reason (required)",
    "Grund für die Notarbeit im Urlaub" : "Reason for the emergency work during vacation",
    "Der Mitarbeiter hat am %s eine laufende Stempelung. Bitte zuerst die Stempelung beenden, dann die ganztägige Abwesenheit genehmigen." : "The employee has a running punch on %s. Please end the punch first, then approve the full-day absence.",
    "Verwerfen" : "Discard",
    "Die laufende Stempelung wird ohne Buchung verworfen. Fortfahren?" : "The running punch will be discarded without booking. Continue?",
    "Stempelung verwerfen" : "Discard punch",
    "Verwerfen fehlgeschlagen." : "Discard failed.",
    "Diese Stempelung ist seit über 24 Stunden offen und lässt sich nicht als einzelner Zeiteintrag buchen. Bitte verwerfen Sie die Stempelung und erfassen Sie die Zeit manuell." : "This punch has been open for more than 24 hours and cannot be booked as a single time entry. Please discard the punch and enter the time manually.",
    "Diese Stempelung ist seit über 24 Stunden offen und lässt sich nicht als einzelner Zeiteintrag buchen. Bitte schließe diesen Dialog, verwirf die Stempelung und erfasse die Zeit manuell." : "This punch has been open for more than 24 hours and cannot be booked as a single time entry. Please close this dialog, discard the punch and enter the time manually.",
    "Ruhend ab" : "Resting from",
    "Ab diesem Tag fällt kein Sollstunden-Zuwachs mehr an. Standard: heute." : "From this day on, no additional target hours accrue. Default: today.",
    "Das Reaktivierungsdatum darf nicht vor dem Ruhend-ab-Datum liegen." : "The reactivation date must not be before the resting-from date.",
    "Ungültiges Datum. Erwartet wird JJJJ-MM-TT." : "Invalid date. Expected YYYY-MM-DD.",
    "Abteilung bearbeiten" : "Edit department",
    "Neue Abteilung" : "New department",
    "Inaktive Abteilungen stehen nicht mehr zur Auswahl." : "Inactive departments are no longer available for selection.",
    "Abteilung aktualisiert" : "Department updated",
    "Abteilung erstellt" : "Department created",
    "Keine Abteilungen" : "No departments",
    "Legen Sie Abteilungen an, um Mitarbeitende Organisationseinheiten zuzuordnen." : "Create departments to assign employees to organisational units.",
    "Abteilung löschen?" : "Delete department?",
    "Möchten Sie die Abteilung \"{name}\" wirklich löschen?" : "Do you really want to delete the department \"{name}\"?",
    "_%n Mitarbeitende/r wird dieser Abteilung entzogen. Die Zeiterfassung bleibt unberührt._::_%n Mitarbeitende werden dieser Abteilung entzogen. Die Zeiterfassung bleibt unberührt._" : ["%n employee will be unassigned from this department. Time tracking is unaffected.","%n employees will be unassigned from this department. Time tracking is unaffected."],
    "Abteilung" : "Department",
    "Ordnet den Mitarbeiter einer Organisationseinheit zu. Rein organisatorisch – unabhängig vom Vorgesetzten und ohne Einfluss auf Berechtigungen." : "Assigns the employee to an organisational unit. Purely organisational — independent of the supervisor and with no effect on permissions.",
    "Keine Abteilung" : "No department",
    "Alle Abteilungen" : "All departments",
    "Keine Mitarbeitenden in dieser Abteilung." : "No employees in this department.",
    "Ohne Abteilung" : "Without department",
    "Abteilungen" : "Departments",
    "Abteilung gelöscht" : "Department deleted",
    "z.B. 45" : "e.g. 45",
    "Ihre gewohnte Pause wird bei neuen Einträgen vorausgefüllt. Sie kann die gesetzliche Mindestpause (§4 ArbZG) nie unterschreiten — bei Bedarf wird automatisch der gesetzliche Wert verwendet. Leer lassen für die gesetzliche Mindestpause." : "Your usual break is prefilled for new entries. It can never fall below the statutory minimum break (§4 ArbZG) — the statutory value is used automatically when needed. Leave empty for the statutory minimum break.",
    "Zeit" : "Time",
    "von {name} am {date}" : "by {name} on {date}",
    "Aus Favoriten entfernen" : "Remove from favorites",
    "Zu Favoriten hinzufügen" : "Add to favorites"
},
"nplurals=2; plural=(n != 1);");
